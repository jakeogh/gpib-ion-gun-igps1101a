...
# Optional pretty print import guarded in CLI layer if desired
try:
...
class HostAscii:
    ...
    def txrx(self, cmd: str, sleep_before_read: float = 0.05) -> str:
...
def verify_expected_device(
    cli: HostAscii, expected_model_substr: str, dbg: DebugLog
) -> IdInfo:
...
def build_indices(
    probe_min: int, probe_max: int, only_known: bool
...
def format_table(idinfo: IdInfo, status: StatusInfo, readings: list[Reading]) -> str:
